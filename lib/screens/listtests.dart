...
class ListTests extends StatefulWidget {
  @override
  _ListTestsState createState() => _ListTestsState();
}
...
class _ListTestsState extends State<ListTests> {
  ...
  getSubjects() async {
    ...
    var mygetter = box.get('testskey');

    if (mygetter == null || mygetter.myData.length == 0) {
      isNull = true;
    } else {
      isNull = false;
      myData = box.get('testskey').myData;
    ...
    return Scaffold(
      ...
      body: isLoading
          ...
          : isNull == true
              ? Center(
                  child: Text(
                  "Add some data !\nIf your test is completed then tick the CheckBox!",
